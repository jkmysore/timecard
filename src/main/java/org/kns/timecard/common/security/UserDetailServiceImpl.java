...
public class UserDetailServiceImpl implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String username)
		...
		try{
			userEntity=this.userService.getTimeCardUserByUsernameorEmail(username);
		...
		/* Handling User Role */
		 ...
		 for(RolesDto roleDto:userEntity.getRole()){
			 for(String role: UserRoles.valueOf(roleDto.getRoleName()).getRoles()){
				 authorities.add(new GrantedAuthorityImpl(role));
			 }
		 }
		 /* Performing Actual Authentication */		 
		 User user=new User(userEntity.getEmail(), userEntity.getPassword(), enabled, accountNonExpired, credentialsNonExpired, accountNonLocked, authorities);
